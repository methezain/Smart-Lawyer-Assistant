...
import { useUpdateAgreementMutation } from "../../../reduxstore/services/AgreementsAPI";
...
const EditAgreement = ({
  ...
  const original = location.state?.contract || null;

  // Derive initial values from existing contract when available
  const initialForm = useMemo(() => {
    ...
    const isKnown = c.caseType && caseTypeKnown.includes(c.caseType);
    const derivedCaseType = isKnown
      ? c.caseType
      : c.caseType
      ? "Other"
      : "Civil";
    const derivedCustom = !isKnown && c.caseType ? c.caseType : "";
    return {
      title: c.title || "",
      caseType: derivedCaseType,
      customCaseType: derivedCustom,
      client: c.client || "",
      ...
      expectedFileDate: c.expectedFileDate || "",
      effectiveDate: c.effectiveDate || "",
      contractDuration: c.contractDuration || "",
      terminationDate: c.terminationDate || "",
      amount: c.amount ?? "",
      currency: c.currency || "PKR",
      terms: Array.isArray(c.terms) && c.terms.length ? c.terms : [""],
  ...
  const [updateAgreement, { isLoading: isSaving }] =
  ...
  const handleSubmit = async (e) => {
    ...
    if (onSubmit) {
      ...
      try {
        await updateAgreement(payload).unwrap();
        goBackToList();
      } catch (err) {
            ...
            <div className="p-4">
              <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
                <label className="text-xs font-medium text-gray-700 mb-3 flex items-center">
                  ...
                  Update Terms <span className="text-red-500 ml-1">*</span>
                </label>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
            ...
            <div className="p-4 text-xs text-gray-700 space-y-2">
              <div className="flex items-start gap-2">
                ...
                <span>
                  Required: Title, Case Type, Status, Duration, Termination
                  Date, Client, Content, Terms.
                </span>
              </div>
              <div className="flex items-start gap-2">
                <i className="ri-calendar-line text-gray-400 mt-0.5"></i>
                <span>Update Effective and Termination dates as needed.</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
            ...
            <div className="p-4 pb-2.5 text-xs text-gray-700 space-y-2">
              ...
              <div className="pt-2 border-t border-gray-100 text-xs text-gray-500">
                Preview of the updated agreement.
              </div>
            </div>
          </div>
        </aside>
