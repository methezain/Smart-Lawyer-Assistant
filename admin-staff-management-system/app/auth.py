"""
JWT Authentication utilities for Staff Registration Service.
Mirrors the Case Management System auth so all microservices share the same contract.
"""
...
# JWT settings - Must match Admin Registration/Auth service
...
def decode_jwt_token(token: str) -> Optional[Dict[str, Any]]:
	...
	Returns:
		Optional[Dict[str, Any]]: Decoded payload or raises HTTPException
	"""
	try:
		logger.info(
			f"Attempting to decode JWT token. Secret key starts with: {SECRET_KEY[:10]}..."
		)
...
def get_current_user(
	...
	try:
		...
		if not all([user_id, username, firm_id]):
			logger.error(
				f"Missing required token fields - User ID: {user_id}, Username: {username}, Firm ID: {firm_id}"
			)
			raise HTTPException(
				status_code=status.HTTP_401_UNAUTHORIZED,
		...
		return JWTTokenData(
			user_id=user_id, username=username, firm_id=firm_id, user_type=user_type
		)
...
def get_user_firm_filter(current_user: JWTTokenData = Depends(get_current_user)) -> int:
	"""Return firm_id for downstream filtering."""
